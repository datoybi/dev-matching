...
따라서 SPA(single page app)은 state만 읽으면 모든게 보여지고 숨겨지게 렌더링해야 한다. 모달창을 특정 상황에서 보여줘야한다면 "해당 버튼을 click시, hide된것을 show해주세요" 가 아니라 this.state = {isModal: true/false}와 같이 상태값을 주어 랜더링 해주는 컴포넌트를 따로 만들어야 한다는 말이다.<br><br>
...
4. 구현하다 보니까 내가 뭘 하고있었는지 자주 까먹고 머리가 엉켰다. 이를 방지하기위해 요구사항 분석과 그걸 어떻게 해결할지를 계속 적고 해결하며 구현해야 겠다<br>
